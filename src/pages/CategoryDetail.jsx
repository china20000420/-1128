...
export default function CategoryDetail() {
  ...
  useEffect(() => {
    loadData()
  }, [planName, stageName, categoryName, subcategoryName])
  ...
  const saveData = async (isAutoSave = false) => {
    ...
    try {
      ...
      await axios.post(`/api/plans/${planName}/stages/${stageName}/categories/${categoryName}/${subcategoryName}`, {
        description,
        rows,
        tokenCountTotal: totals.tokenCountTotal,
        actualTokenTotal: totals.actualTokenTotal
      })
  ...
  const calculateTotals = () => {
    ...
    rows.forEach(row => {
      const tokenCount = parseFloat(row.token_count) || 0
      const actualToken = parseFloat(row.actual_token) || 0
      tokenCountTotal += tokenCount
      actualTokenTotal += actualToken
    })
    ...
    <div>
      <Title level={2}>
        ...
        <span style={{ fontSize: '16px', fontWeight: 'normal', marginLeft: '20px', color: '#666' }}>
          数据集总token累计为：{totals.tokenCountTotal}，实际使用token累计为：{totals.actualTokenTotal}
        </span>
      </Title>
